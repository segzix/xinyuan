...
#define BT_HCI_OP_VS_WRITE_MEMORY				BT_OP(BT_OGF_VS, 0x0017)
struct bt_hci_cp_vs_write_memory {
	uint32_t dst_addr;
	uint8_t  mem_size;
    uint8_t  mem_content[];
} __packed;
...
#if defined(CONFIG_VS_HCI_CMD_MANTLE_V1P0) && (CONFIG_VS_HCI_CMD_MANTLE_V1P0 == 1)
#define BT_HCI_OP_VS_SET_BD_ADDR                BT_OP(BT_OGF_VS, 0x0007)
#elif defined(CONFIG_VS_HCI_CMD_V3) && (CONFIG_VS_HCI_CMD_V3 == 1)
...
#if defined(CONFIG_VS_HCI_CMD_MANTLE_V1P0) && (CONFIG_VS_HCI_CMD_MANTLE_V1P0 == 1)
#define BT_HCI_OP_VS_GET_BD_ADDR                BT_OP(BT_OGF_VS, 0x0008)
#elif defined(CONFIG_VS_HCI_CMD_V3) && (CONFIG_VS_HCI_CMD_V3 == 1)
...
/**
 * Test commands that defined in core spec
 */
...
struct bt_hci_tx_ts_report {
	uint16_t iso_handle;
	uint16_t sdu_seq_num;
	uint32_t sdu_tx_time_stamp;
	uint8_t  sdu_time_offset[3];
	uint16_t tx_seq_num;
	uint32_t tx_time_stamp;
	uint8_t  tx_time_offset[3];
} __packed;
struct bt_hci_evt_vs_le_vs_iso_ts_report {
    uint8_t num_iso;
	struct bt_hci_tx_ts_report iso[0];
} __packed;
#ifdef __cplusplus
}
#endif
