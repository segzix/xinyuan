...
#ifndef VSBT_INCLUDE_BLUETOOTH_AUDIO_TBS_H_
#define VSBT_INCLUDE_BLUETOOTH_AUDIO_TBS_H_
...
#define BT_TBS_FEATURE_HOLD BTBIT(0)
/** Join Call Control Point Opcode supported */
#define BT_TBS_FEATURE_JOIN BTBIT(1)
/** @} */
...
#define BT_TBS_STATUS_FLAG_INBAND_RINGTONE BTBIT(0)
/** Server is in silent mod */
#define BT_TBS_STATUS_FLAG_SILENT_MOD BTBIT(1)
/** @} */
...
#define BT_TBS_CALL_FLAG_OUTGOING BTBIT(0)
/** If set call is withheld, else not withheld */
#define BT_TBS_CALL_FLAG_WITHHELD BTBIT(1)
/** If set call is withheld by network, else provided by network */
#define BT_TBS_CALL_FLAG_WITHHELD_BY_NETWORK BTBIT(2)
/** @} */
 ...
 */
typedef void (*bt_tbs_join_calls_cb)(struct bt_conn *conn, uint8_t call_index_count,
 ...
 */
typedef void (*bt_tbs_client_discover_cb)(struct bt_conn *conn, int err, uint8_t tbs_count,
                                          bool gtbs_found);
 ...
 */
typedef void (*bt_tbs_client_cp_cb)(struct bt_conn *conn, int err, uint8_t inst_index,
                                    uint8_t call_index);
 ...
 */
typedef void (*bt_tbs_client_read_string_cb)(struct bt_conn *conn, int err, uint8_t inst_index,
                                             const char *value);
 ...
 */
typedef void (*bt_tbs_client_read_value_cb)(struct bt_conn *conn, int err, uint8_t inst_index,
                                            uint32_t value);
 ...
 */
typedef void (*bt_tbs_client_termination_reason_cb)(struct bt_conn *conn, int err,
 ...
 */
typedef void (*bt_tbs_client_current_calls_cb)(struct bt_conn *conn, int err, uint8_t inst_index,
                                               uint8_t call_count,
 ...
 *                      the client, so must be copied to be saved.
 */
typedef void (*bt_tbs_client_call_states_cb)(struct bt_conn *conn, int err, uint8_t inst_index,
                                             uint8_t call_count,
 ...
 */
int bt_tbs_client_set_signal_strength_interval(struct bt_conn *conn, uint8_t inst_index,
                                               uint8_t interval);
 ...
 * for this function to be effective.
 */
int bt_tbs_client_join_calls(struct bt_conn *conn, uint8_t inst_index, const uint8_t *call_indexes,
                             uint8_t count);
 ...
 *
 * @param[in]  bearer_index  The index of the Telephone Bearer.
 * @param[in] call_indexes  Array of call indexes.
 *
 * @return int          0 on success, errno value on fail.
 *
 */
int bt_tbs_remote_alert(uint8_t bearer_index, uint8_t call_index);
#ifdef __cplusplus
}
#endif
...
#endif /* VSBT_INCLUDE_BLUETOOTH_AUDIO_TBS_H_ */
