...
#ifndef VSBT_INCLUDE_BLUETOOTH_AUDIO_BAP_
#define VSBT_INCLUDE_BLUETOOTH_AUDIO_BAP_
...
struct bt_bap_scan_delegator_cb {
    ...
    int (*pa_sync_req)(struct bt_conn *conn,
                       const struct bt_bap_scan_delegator_recv_state *recv_state, bool past_avail,
 ...
 * a unicast group shall share the same interval, framing and latency (see @ref bt_bap_qos_cfg).
 *
 ...
 *                           2, this should have the value of BTBIT(1) | BTBIT(2).
...
struct bt_bap_broadcast_assistant_cb {
     ...
     * @param broadcast_id  24-bit broadcast ID.
     */
    void (*scan)(const struct bt_le_scan_recv_info *info, uint32_t broadcast_id, char *adv_name);
...
/**
 * @brief Get the audio direction of the given LE Audio stream.
 *
 * @param stream   Pointer to the constant BAP stream whose direction is to be obtained.
 * @param dir      Pointer to an enum bt_audio_dir variable where the direction will be stored.
 *
 * @return 0 on success, or a negative error code if retrieving endpoint info fails.
 */
int le_audio_stream_dir_get(struct bt_bap_stream const *const stream, enum bt_audio_dir *dir);
/**
 * @brief Reconstruct the ISO data path for the given BAP stream.
 *
 * This function removes the existing ISO data path associated with the stream's
 * connection and sets up a new HCI ISO data path with specified parameters.
 *
 * @param stream   Pointer to the BAP stream whose data path is to be reconstructed.
 *
 * @return 0 on success, or a negative error code on failure.
 */
int bap_stream_datapath_reconstruction(struct bt_bap_stream *stream,
                                       enum bt_audio_dir audio_stream_dir);
#endif // CONFIG_BT_ISO_DUAL_PATH_SUPPORT_HOST
...
#endif /* VSBT_INCLUDE_BLUETOOTH_AUDIO_BAP_ */
