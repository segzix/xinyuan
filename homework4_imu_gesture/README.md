...
```text
../VeriHealthi_QEMU_SDK.202606_preliminary/VeriHealthi_IMU_Dataset
```

该目录按 `pinch`、`clench`、`up`、`down`、`others` 分类存放数据，目录名就是当前 host 评估使用的标准标签。先找一个可运行样本：

```bash
find ../VeriHealthi_QEMU_SDK.202606_preliminary/VeriHealthi_IMU_Dataset -type f -name '*.txt' | head
```

运行单个 IMU 文本文件：

```bash
make run DATA=../VeriHealthi_QEMU_SDK.202606_preliminary/VeriHealthi_IMU_Dataset/pinch/IMU_pinch_right_2026_05_29_14_20_10_ID5.txt
```
...
`make run-all` 默认使用 `../VeriHealthi_QEMU_SDK.202606_preliminary/VeriHealthi_IMU_Dataset`。如果要临时运行其它数据目录，可以覆盖 `DATA_DIR`：
...
```bash
make -B && find ../VeriHealthi_QEMU_SDK.202606_preliminary/VeriHealthi_IMU_Dataset -type f -name '*.txt' | sort | while IFS= read -r f; do printf '\n== %s ==\n' "$f"; build/imu_gesture_host "$f"; done
```
